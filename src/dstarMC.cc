...
dstarMC::dstarMC()
{
    ...
    coplanarity_MC = new TH1D ("coplanarity_MC", "coplanarity_MC",    360, 0, 360);
    ...
    theta_MC   = new TH1D ("theta_MC", "theta_MC",    180,    0,  180);
    ...
    theta_charged_MC   = new TH1D ("theta_charged_MC", "theta_charged_MC",    180,    0,  180);
    theta_neutral   = new GH1 ("theta_neutral", "theta_neutral",    180,    0,  180);
    theta_neutral_MC   = new TH1D ("theta_neutral_MC", "theta_neutra_MCl",    180,    0,  180);
    ...
    dE_E_CB = new  GHistBGSub2("dE_E_CB", "dE_E_CB", 200,  0,  500, 50, 0, 10);
...
void	dstarMC::ProcessEvent()
{
...

    {
        ...
        if(GetTracks()->IsNeutral(0) == GetTracks()->IsCharged(1))
        {
            ...
            {
                cout << "check simulation, event: " << conteggio << " traccia: " << i << " theta MC: " << GetGeant()->GetTrueVector(1).Theta() << endl;

                for (Int_t j = 0; j < GetTagger()->GetNTagged(); j++)
                {
                    if(i==0) {
                        Fillcoplanarity(j, coplanarity, kTRUE);
                        coplanarity_MC->Fill(TMath::Abs(TMath::RadToDeg()*GetGeant()->GetTrueVector(0).Phi()-TMath::RadToDeg()*GetGeant()->GetTrueVector(1).Phi()));
                    }
                    ...
                    theta_MC->Fill(TMath::RadToDeg()*GetGeant()->GetTrueVector(i).Theta());
                    ...
                    if(coplanarity_condition == true)
                    {
                        ...
                        if(GetTracks()->IsNeutral(i))
                        {
                            ...
                            theta_neutral_MC->Fill(TMath::RadToDeg()*GetGeant()->GetTrueVector(i).Theta());
                        ...
                        if(GetTracks()->IsCharged(i))
                        {
                            ...
                            theta_charged_MC->Fill(TMath::RadToDeg()*GetGeant()->GetTrueVector(i).Theta());
...
void	dstarMC::ProcessScalerRead()
{
    // Fill Tagger Scalers
