...
void PPhysics::FillMissingMass1(const GTreeParticle& tree, Int_t particle_index, GH1* gHist, Bool_t TaggerBinning)
{
    for (Int_t i = 0; i < GetTagger()->GetNTagged(); i++)
    {
...
//TLorentzVector_track PPhysics::CalcMissingP4(Int_t particle_index1 ,Int_t particle_index2, Int_t tagger_index)
//{
//    particle	= tree.Particle(particle_index);
//    beam 		= TLorentzVector(0.,0.,GetTagger()->GetTaggedEnergy(tagger_index),GetTagger()->GetTaggedEnergy(tagger_index));
//    missingp4 	= beam + target - particle;
...
void PPhysics::FillMissingMasstrack(Int_t particle_index1, Int_t particle_index2, Int_t tagger_index, GH1* gHist, Bool_t TaggerBinning)
{
//    cout << "qui entro" << endl;
    if(RejectTagged(tagger_index)) return;
    double mass_1 = 0;
    ...
    if(GetTracks()->IsNeutral(particle_index1)==1)
    {
        mass_1 = 939.565;
        mass_2 = 938.272;
    }
    else if(GetTracks()->IsNeutral(particle_index1)==0)
    {
        mass_1 = 938.272;
        mass_2 = 939.565;
    }

    TLorentzVector track1 = GetTracks()->GetVector(particle_index1, mass_1);
    TLorentzVector track2 = GetTracks()->GetVector(particle_index2, mass_2);

    // calc particle time diff
    time = GetTagger()->GetTaggedTime(tagger_index) - GetTracks()->GetTime(particle_index1);
    beam 		= TLorentzVector(0.,0.,GetTagger()->GetTaggedEnergy(tagger_index),GetTagger()->GetTaggedEnergy(tagger_index));

    // calc missing p4
    TLorentzVector missingp4_track	= beam + target - track2 ;
...
void PPhysics::FillMissingMasscharged_neutral(Int_t particle_index0, Int_t particle_index1, Int_t tagger_index, GH1* gHist, GH1* gHist1, Bool_t TaggerBinning)
{
    ...
    if(GetTracks()->IsNeutral(particle_index0)==1 && GetTracks()->IsNeutral(particle_index1)==0)
    {
        ...
        beam 		= TLorentzVector(0.,0.,GetTagger()->GetTaggedEnergy(tagger_index),GetTagger()->GetTaggedEnergy(tagger_index));
        TLorentzVector missingp4_track_neutral	= beam + target - track0;
        TLorentzVector missingp4_track_charged	= beam + target - track1;
        if(TaggerBinning)
        {
            gHist->Fill(missingp4_track_neutral.M(),time, GetTagger()->GetTaggedChannel(tagger_index));
            gHist1->Fill(missingp4_track_charged.M(),time, GetTagger()->GetTaggedChannel(tagger_index));
        }
    ...
    else if(GetTracks()->IsNeutral(particle_index0)==0 && GetTracks()->IsNeutral(particle_index1)==1)
    {
        ...
        time = GetTagger()->GetTaggedTime(tagger_index) - GetTracks()->GetTime(particle_index0);
        beam 		= TLorentzVector(0.,0.,GetTagger()->GetTaggedEnergy(tagger_index),GetTagger()->GetTaggedEnergy(tagger_index));
        TLorentzVector missingp4_track_charged 	= beam + target - track0;
        TLorentzVector missingp4_track_neutral	= beam + target - track1;
        if(TaggerBinning)
        {
            gHist->Fill(missingp4_track_neutral.M(),time, GetTagger()->GetTaggedChannel(tagger_index));
            gHist1->Fill(missingp4_track_charged.M(),time, GetTagger()->GetTaggedChannel(tagger_index));
...
//    cout << "E CB function: " <<  cluster_energy << endl;
//    cout << "E PID function: " <<  veto_energy << endl;
...
void PPhysics::FillTOF_TAPS(Int_t track_index, GHistBGSub2* gHist)
{
    double  cluster_energy;
    double  cluster_time;
    double  cluster_theta;
    double  time_corrected;
    ...
    for (Int_t j = 0; j < GetTagger()->GetNTagged(); j++)
    {
        ...
        {
            if(GetTracks()->HasTAPS(r))
            {
                cluster_energy = GetTracks()->GetClusterEnergy(r);
                cluster_time = GetTracks()->GetTime(r);
                cluster_theta = GetTracks()->GetTheta(r);
                time_corrected = cluster_time*TMath::Cos((cluster_theta)*TMath::DegToRad());
                // cout << cluster_theta << "  "<< TMath::Cos((cluster_theta)*TMath::DegToRad()) << endl;

                Double_t time = GetTagger()->GetTaggedTime(j) -  GetTracks()->GetTime(track_index);
                gHist->Fill(cluster_energy, time_corrected, time);
            }
        }
    }
}
void PPhysics::FillTOF_TAPS(Int_t track_index, Int_t tagger_index, GHistBGSub2* gHist)
{
    double  cluster_energy;
    double  cluster_time;
    double  cluster_theta;
...
void PPhysics::Filldetector(TH1* gHist)
{
    int proton_det =0;
    int neutron_det =0;
    int detector = 0;
    ...
    for(Int_t r=0; r<GetTracks()->GetNTracks(); r++)
    {
        if(GetTracks()->IsCharged(r))
        {
            ...
            else if(GetTracks()->HasCB(r))
            {
                proton_det = 0;
            }
        }
        if(GetTracks()->IsNeutral(r))
        {
            if(GetTracks()->HasTAPS(r))
            {
                neutron_det = 1;
...
//    cout << "detectors: " << detector << endl;
    gHist->Fill(detector);
}
void PPhysics::Fillcluster_size_track(Int_t track_index, GHistBGSub2* gHist)
{
    Double_t cluster_size;
    Double_t cluster_energy;
    for (Int_t j = 0; j < GetTagger()->GetNTagged(); j++)
    {
        cluster_size = GetTracks()->GetClusterSize(track_index);
        cluster_energy = GetTracks()->GetClusterEnergy(track_index);
        Double_t time = GetTagger()->GetTaggedTime(j) - GetTracks()->GetTime(track_index);
        gHist->Fill(cluster_energy, cluster_size, time);
    }
}

void PPhysics::Fillcluster_size_track(Int_t track_index, Int_t tagger_index, GHistBGSub2* gHist)
{
    Double_t cluster_size;
    Double_t cluster_energy;
